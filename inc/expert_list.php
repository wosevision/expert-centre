...
<section class="row">
  ...
  <aside class="medium-3 columns section_content_wrapper stickyNav">
    ...
    <div class="section-container vertical-nav" data-section data-options="deep_linking: false; one_up: true">
      ...
      <section class="section">
        <label for="expert-search" class="search_label">
          <h4>Keyword</h4>
        </label>
          ...
          <div class="row collapse">
            <div class="small-10 columns">
              ...
              <input class="textfield" type="text" value="
                <?php echo $filter_keyword; ?>" maxlength="256" id="expert-search" name="q">
              <?php else: ?>
              <input class="textfield" type="text" placeholder="Enter search term" maxlength="256" id="expert-search" name="q" value="">
              <?php endif ?>
            </div>
            <div class="small-2 columns">
              <button type="button" class="button postfix" onclick="loadSearch();">
              <span class="fa fa-search" title="Search" aria-hidden="true"></span>
              </button>
            </div>
          </div>
        </form>
    ...
    <div id="expert-list-inner">
      ...
      <div class="row">
        <dl class="sub-nav">
          ...
          <dd class="
            <?php echo ($_GET['s'] == 'name') ? 'active' : ''; ?>">
            <a class="expert-filter" href="?
              <?php echo (isset($_GET['n'])) ? 'n='.$_GET['n'].'&' : ''; ?>s=name">A - Z
            </a>
          </dd>
          <dd class="
            <?php echo ($_GET['s'] == 'featured') ? 'active' : ''; ?>">
            <a class="expert-filter" href="?
              <?php echo (isset($_GET['n'])) ? 'n='.$_GET['n'].'&' : ''; ?>s=featured">Featured
            </a>
          </dd>
          <dd class="
            <?php echo ($_GET['s'] == 'updated') ? 'active' : ''; ?>">
            <a class="expert-filter" href="?
              <?php echo (isset($_GET['n'])) ? 'n='.$_GET['n'].'&' : ''; ?>s=updated">Recently updated
            </a>
          </dd>
        </dl>
        <dl class="sub-nav">
          <dt>Page size:</dt>
          <dd class="
            <?php echo ($_GET['n'] == 5) ? 'active' : ''; ?>">
            <a class="expert-filter" href="?
              <?php echo (isset($_GET['s'])) ? 's='.$_GET['s'].'&' : ''; ?>n=5">5
            </a>
          </dd>
          <dd class="
            <?php echo ($_GET['n'] == 10) ? 'active' : ''; ?>">
            <a class="expert-filter" href="?
              <?php echo (isset($_GET['s'])) ? 's='.$_GET['s'].'&' : ''; ?>n=10">10
            </a>
          </dd>
          <dd class="
            <?php echo ($_GET['n'] == 20) ? 'active' : ''; ?>">
            <a class="expert-filter" href="?
              <?php echo (isset($_GET['s'])) ? 's='.$_GET['s'].'&' : ''; ?>n=20">20
            </a>
          </dd>
        </dl>
      </div>
      ...
      <?php foreach($experts as $key=>$value):
        ...
        ?>
      <article class="row expert" data-topics="
        <?php echo $topicData; ?>">
        <div class="small-3 columns">
          <?php if(isset($value->url)): ?>
          <a href="?expert=
            <?php echo $value->username; ?>">
          <img class="expert_avatar" src="
            <?php echo $value->url; ?>" alt="
            <?php echo $fullname; ?>"/>
          </a>
          <?php endif ?>
        </div>
        <div class="small-9 columns">
          <h4>
            ...
            <?php echo $value->country == 'CA' ? 'CAN' : $value->country; ?>
            </small>
            </strong>
          </h4>
          <p>
            <?php echo $value->tagline; ?>
          </p>
          <a href="?expert=
            <?php echo $value->username; ?>" class="button" title="View 
            <?php echo $fullname; ?>'s profile">View Profile
          </a>&nbsp;
          <a href="//expertfile.com/experts/inquire/
            <?php echo urlencode($value->username); ?>" class="buttonWhite" title="Inquire on expertfile.com" target="new">
          <span class="fa fa-envelope" aria-hidden="true"></span> Inquire
          </a>
          <hr/>
          ...
          <div class="row">
            <div class="small-12 columns">
              <?php foreach($value->topics as $k=>$t) {
                echo '
                                            <a href="?q='.urlencode($t->name).'" class="label expert-filter">'.$t->name.'</a>';
                } 
                ?>
            </div>
          </div>
  ...
  <aside class="medium-3 columns section_content_wrapper text-center stickyNav">
    <h4>Can't find who you're
      <br aria-hidden="true" />looking for?
      <br/>
      <small>Contact us and we'll help you find the perfect expert!</small>
    </h4>
    <hr/>
    <div class="row">
      <div class="small-6 medium-12 columns">
        <p>
          <strong>Melissa Levy</strong>
          <br/>
    ...
    <p>General inquiries to the department can also be emailed to 
      <a href="mailto:communications@uoit.ca">communications@uoit.ca</a>.
    </p>
  </aside>
</section>
